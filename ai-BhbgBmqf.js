import { c as simFormatId, r as DEFAULT_WEIGHTS, s as MANIFEST } from "./config-BvcHG8gK.js";
...
var Mech = class {
	...
	damage(att, def, attSide, move, f) {
		const key = `${attSide}|${att.key}|${att.species}|${JSON.stringify(att.boosts)}|${att.status}|${att.itemGone ? 1 : 0}|${att.item ?? ""}|${Math.round(att.hp / 10)}|${def.key}|${def.species}|${JSON.stringify(def.boosts)}|${def.status}|${def.itemGone ? 1 : 0}|${def.item ?? ""}|${Math.round(def.hp / 10)}|${move}|${f.weather}|${f.terrain}|${JSON.stringify(f.screens[1 - attSide])}`;
		...
		try {
			...
			if (a && d) {
				...
				this.roll = ROLL_AVG;
				const avg = this.b.actions.getDamage(a, d, mv, true);
				this.roll = ROLL_MIN;
				const min = this.b.actions.getDamage(a, d, this.b.dex.getActiveMove(toID(move)), true);
				this.roll = ROLL_AVG;
...
function typesOf(m) {
	return dex.species.get(m.set.species).types;
}
