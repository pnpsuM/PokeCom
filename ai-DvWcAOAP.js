import { c as simFormatId, r as DEFAULT_WEIGHTS, s as MANIFEST } from "./config-MbxaO8VP.js";
...
/** 샘플 파티 목록 (처음 쓸 때 합법성 검사, 규칙에 안 맞는 파티는 뺀다) */
function curatedParties() {
	curatedCache ??= MANIFEST.data.curated.flatMap((f) => f.parties.map((p) => ({
		...
		url: p.url,
		source: f.source,
		sets: p.sets.map((s) => normalize({
			...s,
			evs: Object.fromEntries(STATS.map((k, i) => [k, s.sp[i] ?? 0]))
		}))
	}))).filter((p) => !validateTeam(p.sets));
	return curatedCache;
}
var curatedLabel = (p) => `${p.season}${p.rank ? " " + p.rank : ""} ${p.title}`;
var cloneSet = (s) => ({
	...s,
	evs: { ...s.evs },
	ivs: { ...s.ivs },
...
/** 상대 파티: 일정 확률로 샘플 파티, 아니면 랜덤 세트 파티 */
var OpponentSource = {
	label: `구박사 샘플 파티 ${Math.round(MANIFEST.data.curatedOpponentRate * 100)}% · 랜덤 세트`,
	generate() {
		if (curatedParties().length && Math.random() < MANIFEST.data.curatedOpponentRate) {
			const p = CuratedTeamSource.pick();
			return {
				team: p.sets.map(cloneSet),
				party: p
			};
		}
		return {
			team: RandomTeamSource.generate(),
			party: null
		};
	}
};
/** 종 하나의 샘플 세트들: 사람이 만든 샘플(구박사) 먼저, 그 다음 역할별 랜덤 세트 */
...
//#endregion
export { koAbilityDesc as A, BattleStream as B, STAT_SHORT as C, iga as D, euro as E, koMoveDesc as F, getPlayerStreams as H, koNature as I, koSpecies as L, koItem as M, koItemDesc as N, josa as O, koMove as P, koType as R, STAT_KO as S, eun as T, toID as U, Teams as V, calcStat as _, blankSet as a, CATEGORY_KO as b, exportTeam as c, sampleSets as d, statTotal as f, STATS as g, FORMAT as h, SPARE_ITEMS as i, koEffect as j, koAbility as k, importTeam as l, validateTeam as m, OpponentSource as n, curatedLabel as o, validateSet as p, RandomTeamSource as r, curatedParties as s, makeAgent as t, koProblem as u, dex as v, eul as w, STATUS_KO as x, isPickable as y, matchKo as z };
