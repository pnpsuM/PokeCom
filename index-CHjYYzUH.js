...
import { a as GRADES, i as PROFILES, n as CHANGELOG, o as RATING_POLICY, s as MANIFEST, t as APP_VERSION } from "./config-MbxaO8VP.js";
import { A as koAbilityDesc, B as BattleStream, C as STAT_SHORT, D as iga, E as euro, F as koMoveDesc, H as getPlayerStreams, I as koNature, L as koSpecies, M as koItem, N as koItemDesc, O as josa, P as koMove, R as koType, S as STAT_KO, T as eun, U as toID$1, V as Teams, _ as calcStat, a as blankSet, b as CATEGORY_KO, c as exportTeam, d as sampleSets, f as statTotal, g as STATS, h as FORMAT, i as SPARE_ITEMS, j as koEffect, k as koAbility, l as importTeam, m as validateTeam, n as OpponentSource, o as curatedLabel, p as validateSet, r as RandomTeamSource, s as curatedParties, t as makeAgent, u as koProblem, v as dex, w as eul, x as STATUS_KO, y as isPickable, z as matchKo } from "./ai-DvWcAOAP.js";
...
function expandable(el, label = "설명 펼치기") {
	el.classList.add("xp");
...
async function ensure(species, facing, allowDownload = true) {
	await loadIndex();
	const cs = candidates(toID$1(species), facing);
...
function monRow(set, right, onClick) {
	...
	}, thumb(set.species), h("div", null, h("div", { class: "nm" }, koSpecies(set.species), " ", ...speciesTypes(set.species).map(typeTag)), hscroll(h("div", { class: "sub" }, sub)), hscroll(h("div", { class: "sub" }, set.moves.map(koMove).join(" / ")))), right ?? h("span"));
...
function moveCard(move, pp, typeInfo) {
	...
	return h("div", {
		...
		class: "row",
		style: "justify-content:space-between"
	}, h("b", null, koMove(move)), h("span", null, typeTag(i.type), " ", h("span", { class: "muted" }, CATEGORY_KO[i.category] ?? i.category))), typeInfo?.note ? h("div", { class: "typenote" }, typeInfo.note) : null, h("div", {
		class: "muted",
...
function ladderCard(onLadder) {
	...
	const fill = (st) => {
		...
		}, h("div", null, h("div", { class: "eyebrow" }, "래더 레이팅"), h("div", { class: "rating" }, h("span", { class: "rating-num" }, String(st.rating)))), gradeBadge(st.rating)), h("div", { class: "muted" }, `${st.wins}승 ${st.losses}패${st.ties ? ` ${st.ties}무` : ""} · 최고 ${st.peak}` + (streak ? ` · ${streak}` : "") + (nx ? ` · ${nx.name}까지 ${nx.min - st.rating}` : "") + (st.games < 20 ? ` · 배치 ${st.games}/20판 (변동 큼)` : "")), h("div", { class: "row" }, h("button", {
			class: "btn primary grow",
			type: "button",
...
function openRecords() {
	sheet((close) => {
		...
		Promise.all([loadState(), loadGames()]).then(([st, games]) => {
			...
			body.append(h("div", { class: "stats" }, h("div", null, h("b", { class: "rating-num" }, String(st.rating)), h("span", null, "현재")), h("div", null, h("b", { class: "rating-num" }, String(st.peak)), h("span", null, "최고")), h("div", null, h("b", null, `${rate}%`), h("span", null, `${st.wins}승 ${st.losses}패`)), h("div", null, h("b", null, String(st.bestStreak)), h("span", null, "최다 연승"))), h("section", {
				class: "panel",
				style: "padding:10px"
...
function gameRow(g) {
	...
	return h("li", { class: `g ${g.result}` }, h("span", { class: "res" }, g.result === "W" ? "승" : g.result === "L" ? "패" : "무"), h("div", null, h("div", null, `${g.opp.label} `, h("span", { class: "muted" }, `R${g.opp.rating}`)), hscroll(h("div", { class: "muted" }, `${g.myPicks.map(koSpecies).join("·")} vs ${g.oppTeam.map(koSpecies).join("·")}`))), h("div", { class: "delta" }, h("b", { class: g.delta >= 0 ? "up" : "down" }, `${g.delta >= 0 ? "+" : ""}${g.delta}`), h("span", { class: "muted" }, `${g.after} · ${when}`)));
}
//#endregion
...
var BattleView = class {
	...
	upsert(sideId, nick, details) {
		...
		let m = side.team.find((x) => x.nick === nick);
		if (!m) {
			m = side.team.find((x) => !x.revealed && (x.species === d.species || toID$1(x.species) === toID$1(d.species.split("-")[0]) || x.species.endsWith("-*") && d.species.startsWith(x.species.slice(0, -2))));
			if (m) m.nick = nick;
		}
		if (!m) {
			m = {
		...
		switch (cmd) {
			...
			case "-sethp": {
				...
				if (cmd === "-damage" && kw.from) {
					const src = toID$1(kw.from.replace(/^\w+:\s*/, ""));
					const msg = {
				...
				} else if (cmd === "-heal" && kw.from && toID$1(kw.from) === "drain") this.push("text", `${of ? of + "의 " : ""}체력을 흡수했다!`, sid);
			...
			case "-start": {
				...
				const msg = {
					...
					protosynthesis: `${w}의 ${iga(STAT_KO[toID$1(args[2] ?? "")] ?? "능력")} 올라갔다! (고대활성)`,
					quarkdrive: `${w}의 ${iga(STAT_KO[toID$1(args[2] ?? "")] ?? "능력")} 올라갔다! (쿼크차지)`
				};
			...
			case "-enditem": {
				...
				else if (kw.from && /stealeat|bugbite|pluck/i.test(toID$1(kw.from))) this.push("text", `${w}의 ${iga(it)} 먹혀 버렸다!`);
...
function typeShift(pokemon, move, abilityId, used, currentTypes) {
	...
	return {
		abilityId,
		type: move.type,
		stab: move.category !== "Status" && !currentTypes.includes(move.type)
	};
}
/**
...
function resolveMoveType(battle, pokemon, target, moveId, abilityOverride) {
	...
	const ability = toID$1(abilityOverride ?? pokemon.ability);
	const used = abilityOverride && toID$1(abilityOverride) !== toID$1(pokemon.ability) ? false : !!pokemon.abilityState?.[ability];
	...
	try {
		if (abilityOverride) pokemon.ability = toID$1(abilityOverride);
		...
		if (move.type !== base.type) {
			if (move.type !== afterMove) {
				out.cause = "ability";
				out.abilityId = toID$1(pokemon.ability);
			} else out.cause = "move";
		}
...
var TERRAIN_COLOR = {
	electricterrain: "#f2cf1d",
	grassyterrain: "#5fae3c",
	mistyterrain: "#f0a8d0",
	psychicterrain: "#c070e0"
};
var toID = (s) => (s ?? "").toLowerCase().replace(/[^a-z0-9]/g, "");
var sideOf = (ident) => /^p1/.test(ident) ? "p1" : /^p2/.test(ident) ? "p2" : null;
var BattleFx = class {
	inner;
	spr;
	...
	play(line) {
		...
		if (s && (cmd === "-crit" || cmd === "-supereffective" || cmd === "-resisted")) {
			...
			else p.eff = cmd === "-supereffective" ? 2 : .5;
			return;
		}
		if (!this.on) {
		...
		switch (cmd) {
			...
			case "-start":
				...
				break;
			case "-end": if (s && toID((args[1] ?? "").replace(/^move:\s*/, "")) === "substitute") this.dollOut(s);
		}
	}
	...
	run(el, frames, ms, easing = "ease-out") {
		const d = Math.round(Math.min(600, ms) * this.k);
		if (!d || typeof el.animate !== "function") return null;
	...
	moveUse(user, target, moveName) {
		...
		if (mv.category === "Physical" && !this.reduced) {
			const dx = (t.x - u.x) * .16, dy = (t.y - u.y) * .16;
			this.run(this.spr[user], [
				{ translate: "0 0" },
				{
			...
			const frames = this.reduced ? [
				...
				translate: `${t.x - u.x}px ${t.y - u.y}px`,
				scale: "1.2"
			}];
	...
	hit(side, eff, crit) {
		...
		if (!this.reduced) this.run(sp, [
			...
			-amp * .4,
			0
		].map((x) => ({ translate: `${x}px 0` })), eff >= 2 ? 360 : 260, "linear");
		this.run(sp, [
			{ filter: "brightness(1)" },
			{ filter: `brightness(${eff >= 2 ? 2.2 : 1.7})` },
			{ filter: "brightness(1)" }
		], 220);
		if (crit) {
			const f = this.temp("flash", "inset:0;background:#fff");
			this.drop(f, this.run(f, [{ opacity: .75 }, { opacity: 0 }], 180));
		}
	}
	stat(side, up, n) {
		const c = this.center(side);
		const color = up ? "#4aa8ff" : "#ff5a4a";
		for (let wave = 0; wave < n; wave++) for (let i = 0; i < 5; i++) {
			const x = c.x + (i - 2) * 12 + (wave % 2 ? 6 : 0);
			const y0 = up ? c.y + c.h * .3 : c.y - c.h * .35;
			const p = this.temp(up ? "pt up" : "pt down", `left:${x}px;top:${y0}px;background:${color}`);
			const dy = up ? -c.h * .55 : c.h * .55;
			const a = this.reduced ? this.run(p, [
				{ opacity: 0 },
				{ opacity: .9 },
				{ opacity: 0 }
			], 360) : p.animate([
				...
				duration: Math.round(360 * this.k),
				delay: Math.round((wave * 110 + i * 25) * this.k),
				easing: "ease-out",
				fill: "backwards"
			});
	...
	tint(side, color) {
		this.run(this.spr[side], [
			{ filter: "none" },
			{ filter: `drop-shadow(0 0 8px ${color}) saturate(1.4)` },
			{ filter: "none" }
		], 450);
	}
	sparkle(side, color) {
		const c = this.center(side);
		for (let i = 0; i < 6; i++) {
			const p = this.temp("pt spark", `left:${c.x + (i - 2.5) * 11}px;top:${c.y + c.h * .25}px;background:${color}`);
			const a = this.reduced ? this.run(p, [
				{ opacity: 0 },
				{ opacity: 1 },
				{ opacity: 0 }
				...
				{
					translate: `0 ${-c.h * .5}px`,
					opacity: 0,
					scale: "1"
				}
			], {
				duration: Math.round(420 * this.k),
				delay: Math.round(i * 30 * this.k),
				easing: "ease-out",
				fill: "backwards"
			});
			if (a && !this.anims.has(a)) {
...
/** 변환자재·리베로 문구: '변환자재: 쓰면 물 타입이 된다 (자속 보정 1.5배)' */
...
/** 정보창의 능력 변화 표시: 공↑2 스↓1 */
function rankRow(boosts) {
	const list = Object.entries(boosts).filter(([, v]) => v);
	if (!list.length) return null;
	return h("div", {
		class: "ranks",
		"aria-label": "능력 변화"
	}, list.map(([k, v]) => h("span", { class: v > 0 ? "up" : "down" }, `${RANK_KO[k] ?? k}${v > 0 ? "↑" : "↓"}${Math.abs(v)}`)));
}
var PRACTICE = {
...
function startBattle(team, opts = {}) {
	...
	const gen = o.opp ? {
		team: o.opp,
		party: o.oppParty ?? null
	} : OpponentSource.generate();
...
var BattleController = class {
	...
	mountBattle() {
		...
		}), this.sprEl.p2, this.sprEl.p1, this.infoEl.p2, this.infoEl.p1);
	...
	renderScene() {
		...
		for (const sid of ["p1", "p2"]) {
			...
			clear(info);
			info.append(h("div", { class: "nm" }, h("b", null, koSpecies(m.species)), h("span", null, `${m.gender === "M" ? "♂" : m.gender === "F" ? "♀" : ""} Lv${m.level}`)), h("div", { class: "hpbar" }, h("em", null, "HP"), h("div", null, bar)), h("div", { class: "meta" }, m.status ? h("span", { class: `st ${m.status}` }, STATUS_KO[m.status] ?? m.status) : h("span"), h("span", null, sid === "p1" ? `${m.hp}/${m.maxhp}` : `${p}%`)), ...[rankRow(st.sides[sid].boosts)].filter((x) => !!x));
	...
	showCommands(req) {
		...
		const preview = this.session.previewMoveTypes(this.megaOn) ?? [];
		const moves = h("div", { class: "moves" }, act.moves.map((m, i) => {
			const ti = preview.find((x) => x.id === toID$1(m.id)) ?? preview[i];
			...
			}, h("b", null, koMove(m.move)), h("small", null, `${koType(type)}${note ? "*" : ""} · ${m.maxpp ? `${m.pp}/${m.maxpp}` : "-"}`, eff == null ? null : h("span", { class: `eff ${badgeFor(eff).cls}` }, badgeFor(eff).label), stabTag));
			b.style.flex = "1";
			return h("div", { style: "position:relative;display:flex" }, b, h("button", {
				...
				on: { click: (e) => {
					e.stopPropagation();
					sheet((close) => [moveCard(m.move, `${m.pp}/${m.maxpp}`, {
						type,
						note
					}), h("button", {
						class: "btn",
			...
			const draw = () => {
				...
				if (tab === "moves") {
					...
					if (foe) {
						...
						body.append(h("h2", null, `상대 ${foeName}`), h("dl", { class: "kv" }, h("dt", null, "타입"), h("dd", null, ...(st.sides.p2.types ?? speciesTypes(foe.species)).map(typeTag), st.sides.p2.types ? h("div", { class: "muted" }, `바뀐 타입 (원래 ${speciesTypes(foe.species).map(koType).join("·")})`) : null), h("dt", null, "특성"), h("dd", null, foe.ability ? koAbility(foe.ability) : `미공개 (가능: ${possible})`, foe.ability ? h("div", { class: "muted" }, koAbilityDesc(foe.ability)) : null), h("dt", null, "도구"), h("dd", null, foe.item === void 0 ? "미공개" : foe.item ? koItem(foe.item) : "없음")));
					}
				} else {
					for (const sid of ["p1", "p2"]) {
						const side = st.sides[sid];
	...
	async showEnd() {
		...
		if (this.oppParty) this.cmdEl.append(h("div", {
			class: "muted",
			style: "font-size:12px"
		}, `상대 파티: 구박사 샘플 · ${curatedLabel(this.oppParty)}`));
...
function openSettings(onClose) {
	sheet((close) => {
		...
		return [
			...
			h("section", { class: "panel" }, h("h2", null, "배틀 연출"), seg("배틀 연출", [
				["on", "켜기"],
				["reduced", "움직임 줄이기"],
				["off", "끄기"]
			], settings.effects ?? "on", (v) => {
				settings.effects = v;
				saveSettings();
			}), h("div", {
				class: "muted",
				style: "font-size:12px"
			}, "기술·피격·능력 변화·상태이상·회복·메가진화·방어·날씨 연출. 화면을 누르면 바로 넘어간다. 메시지 속도가 즉시면 연출도 없다.")),
...
function renderHome() {
	...
	app.appendChild(h("div", { class: "page" }, h("header", {
		class: "row",
		style: "justify-content:space-between;align-items:flex-end"
	}, h("div", null, h("div", { class: "eyebrow" }, `POKECOM · v${APP_VERSION}`), h("h1", null, "컴까기")), h("button", {
		class: "btn small",
		type: "button",
		"aria-label": "설정",
	...
	}, "다시 뽑기") : null)), h("section", { class: "panel" }, h("h2", null, "상대"), h("div", { class: "muted" }, `파티: ${OpponentSource.label} (Showdown Champions 랜덤 배틀 데이터 + 역할별 SP 배분) · AI: 래더는 내 레이팅에 맞는 단계(초급~상급), 연습은 랜덤 AI`), h("button", {
		class: "btn ghost",
		type: "button",
...
function renderTeamEditor(id, openIndex) {
	...
	app.appendChild(h("div", { class: "page" }, h("div", { class: "topbar" }, h("button", {
		...
		class: "btn small",
		type: "button",
		on: { click: () => curatedDialog((p) => {
			team.sets = p.sets.map(clone);
			if (!existing && (!team.name.trim() || team.name === "새 파티")) team.name = `${p.title} (구박사)`;
			dirty = true;
			draw();
			nameIn.value = team.name;
		}) }
	}, "구박사 파티 불러오기"), existing ? h("button", {
		class: "btn small danger",
		type: "button",
...
/** 구박사 샘플 파티 목록 → 고르면 6마리를 그 파티로 바꾼다 */
	...
	sheet((close) => {
		...
		return [
			...
			h("div", {
				class: "muted",
				style: "font-size:12px"
			}, "유튜브 “구박사의 포켓몬 챔피언스 연구소” 영상의 샘플 분석 화면을 옮겼다. 누르면 지금 파티 6마리를 바꾼다."),
			h("div", { class: "curated" }, parties.map((p) => h("div", {
				class: "panel",
				style: "padding:10px;gap:6px"
			}, h("div", {
				class: "row",
				style: "justify-content:space-between;align-items:flex-start"
			}, h("div", null, h("b", null, p.title), h("div", {
				class: "muted",
				style: "font-size:12px"
			}, curatedLabel(p).replace(p.title, "").trim())), h("a", {
				class: "btn small ghost",
				href: p.url,
				target: "_blank",
				rel: "noopener"
			}, "영상")), h("div", { class: "thumbs" }, p.sets.map((s) => thumb(s.species))), h("div", { style: "font-size:13px" }, p.sets.map((s) => koSpecies(s.species)).join(" · ")), h("button", {
				class: "btn primary small",
				type: "button",
				on: { click: () => confirmSheet(`'${p.title}' 파티로 6마리를 바꿀까?`, "바꾸기", () => {
					close();
					onPick(p);
					toast("샘플 파티를 불러왔다");
				}, false) }
...
function firstSample(species, others = []) {
	const used = new Set(others.map((s) => toID$1(s.item)).filter(Boolean));
	const samples = sampleSets(species);
	const ok = samples.find((s) => !used.has(toID$1(s.set.item)));
	if (ok) return clone(ok.set);
	const set = samples[0] ? clone(samples[0].set) : blankSet(species);
	if (used.has(toID$1(set.item))) set.item = SPARE_ITEMS.find((i) => !used.has(toID$1(i))) ?? "";
	return set;
}
var GEN_RANGES = [
	0,
...
function allPickable() {
	...
	for (const st of stones) for (const b of Object.keys(st.megaStone)) megaBases.add(toID$1(b));
	...
	return new Promise((resolve) => {
		...
		const draw = () => {
			...
			for (const a of [...new Set(Object.values(sp.abilities))]) abil.appendChild(h("button", {
				type: "button",
				"aria-pressed": String(toID$1(a) === toID$1(set.ability)),
				on: { click: () => {
					set.ability = a;
					draw();
				} }
...
function pickMove(set, slot) {
	return new Promise((resolve) => {
		...
		sheet((close) => {
			...
			const others = new Set(set.moves.filter((_, i) => i !== slot).map(toID$1));
			const tbody = h("tbody");
			const count = h("span", { class: "grid-count" });
			const q = h("input", {
				class: "text",
			...
			const draw = () => {
				...
				for (const m of rows) {
					...
					tbody.appendChild(h("tr", {
						class: (toID$1(current) === m.id ? "sel" : "") + (dup ? " dim" : ""),
						tabindex: 0,
						on: {
							click: () => {
								if (dup) {
					...
					}, h("td", null, koMove(m.name), expandable(h("div", { class: "movedesc" }, koMoveDesc(m.name) || ""), `${koMove(m.name)} 설명 펼치기`)), h("td", null, typeTag(m.type)), h("td", null, CATEGORY_KO[m.category]), h("td", { class: "num" }, m.basePower || "-"), h("td", { class: "num" }, m.accuracy === true ? "-" : m.accuracy)));
				}
			};
			q.addEventListener("input", () => {
				f.q = q.value;
...
function pickItem(set, otherSets) {
	return new Promise((resolve) => {
		...
		sheet((close) => {
			...
			const used = new Set(otherSets.map((s) => toID$1(s.item)).filter(Boolean));
			...
			const draw = () => {
				...
				else if (itemTab === "mega") rows = all.filter((i) => i.megaStone && Object.keys(i.megaStone).some((b) => toID$1(b) === species.id));
				...
				for (const i of rows) {
					...
					const wrongMega = i.megaStone && !Object.keys(i.megaStone).some((b) => toID$1(b) === species.id);
					grid.appendChild(h("button", {
						class: "itemcell" + (toID$1(set.item) === i.id ? " sel" : "") + (i.megaStone ? " mega" : ""),
						type: "button",
